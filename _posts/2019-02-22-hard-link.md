...
c:\test\hello.txt 라는 파일이 있다면, 해당 파일은 NTFS 파일시스템 볼륨(C드라이브)내 어딘가에 무명으로 존재하고 있고, 우리는 해당 파일을 c:\test\hello.txt라는 이름으로 링크하여 참조하고 있는 것입니다. 

하드링크를 이용하면 해당 파일을 참조하는 또 다른 이름을 만들 수 있습니다. 예를 들어 기존파일(c:\test\hello.txt)에 대해 하드링크(c:\test\bye.txt)를 만들어 동일한 파일을 참조할 수 있습니다.  

기존파일(hello.txt)과 하드링크(bye.txt)가 같은 파일을 참조하고 있을 때, 몇 가지 일어날 수 있는 경우를 살펴보겠습니다. 

* 파일 수정의 경우
    * hello.txt와 bye.txt는 모두 같은 파일에 대한 링크라서 둘 중 어떤 경로를 통해 수정하더라도 같은 결과를 만듭니다.
    * hello.txt를 수정한 다음 bye.txt 내용을 보면 수정된 내용과 동일한 것을 알 수 있습니다. 물론 역도 마찬가지입니다. 
* 파일 삭제의 경우
    * 파일은 링크 참조 카운트를 내부적으로 유지하고 있습니다. 예제의 경우 링크 참조 카운트는 2가 될 것입니다.
    * 하나의 링크(파일경로)를 삭제하면 그것이 가리키는 파일의 링크 참조카운트가 1씩 감소하고, 참조카운트가 0이 되면 최종적으로 파일이 삭제됩니다.
    * 따라서 기존파일(hello.txt)을 삭제하더라도 하드링크(bye.txt)를 통해 파일을 계속 참조할 수 있습니다. 
* 파일의 권한을 변경하려는 경우
    * 기존 링크(hello.txt)나 새로 생성된 하드링크(bye.txt) 둘 중 어떤 링크를 통해 권한을 바꾸어도 둘의 결과는 동일합니다. 링크가 가리키는 파일의 권한이 변경되는 것이죠. 
    * 탐색기를 열어 hello.txt 파일의 권한을 수정한 후 bye.txt의 권한을 확인해보세요. 동일함을 확인할 수 있을 겁니다. 역도 마찬가지입니다.
...
* 디렉토리간에는 하드링크를 만들 수 없습니다. (C:\hello linked to C:\bye)
* 다른 볼륨간에 하드링크를 만들 수 없습니다. (C:\test\hello.txt linked to D:\test\bye.txt)
...
![Forest](https://www.welivesecurity.com/wp-content/uploads/2018/09/Sandbox-Tweet-Edited.jpg) Source: welivesecurity.com
