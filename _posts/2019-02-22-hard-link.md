...
실제 권한 상승 익스플로잇(공격코드)에서는 하드링크를 어떻게 활용되는지 살펴보겠습니다.

살펴볼 취약점은 2018년 말에 발표된 **태스크 스케쥴러 서비스**의 권한상승(EoP) 취약점입니다. 이 취약점은 많은 이슈를 모았습니다. 그 이유는 제작자(sandboxescaper)가 제로데이를 그대로 공개해 버렸기 때문입니다. 또한 APT 그룹에서 자신들의 오퍼레이션에 즉시 활용할 수 있을 정도의 제로데이였기 때문에(실제로 APT그룹에서 활용하였다는 사실이 밝혀졌습니다.), 많은 시스템들이 위험에 노출되어 버린 상황이 되어 더 이슈가 되었죠.

이슈가 된 만큼 많은 분석 보고서들을 인터넷에서 찾아볼 수 있습니다. 이 글에서는 취약점 자체에 대해선 대략적으로 살펴볼 것이고, 해당 취약점을 통해 권한상승 익스플로잇을 개발하는 데 있어서 하드링크가 어떻게 활용되었는지에 대해 초점을 두고 얘기하려 합니다. 

일단 취약점이 있었던 태스크 스케쥴러 서비스는 윈도우즈에서 가장 높은 권한인 시스템(SYSTEM)권한으로 실행됩니다. 이 서비스는 외부 프로세스의 요청을 받기 위해서 API 인터페이스(via *ALPC*)를 열어두었습니다. 

API 세트 중에서 *SchRpcSetSecurity*라는 이름의 API가 취약점의 원인이 됩니다. 이 API의 역할은 c:\windows\tasks 폴더에 있는 파일의 DACL(퍼미션이라 생각해주세요)을 제어할 수 있는 것인데요. 
