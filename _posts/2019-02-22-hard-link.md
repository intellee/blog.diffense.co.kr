...
BOOL CreateHardLinkW(LPCWSTR lpFileName, LPCWSTR lpExistingFileName) {
    ...
    // 대상파일을 Open한다.(FILE_WRITE_ATTRIBUTES 권한을 요구)
...
위 코드를 보면, 하드링크를 생성하기 위해 `NtOpenFile`과 `NtSetInformationFile`을 사용하고 있음을 알 수 있습니다. `NtOpenFile`로 대상파일을 오픈하고나서, `NtSetInformationFile`을 호출하여 윈도우즈OS에 하드링크 생성을 요청하는 2단계로 이루어져 있는 것이죠.

여기서 눈여겨 볼 점은 대상파일을 Open하는 데 `FILE_WRITE_ATTRIBUTES`을 인자로 사용한 것인데요. 만약 사용자가 대상파일에 대해 `FILE_WRITE_ATTRIBUTES` 열기 권한이 없으면, `NtOpenFile` 호출은 실패하게 되고 결국 `CreateHardLink` 호출은 실패하게 됩니다.
...
James Forshaw는 이 지점에서 FILE_WRITE_ATTRIBUTES 없이 대상파일을 Open(`NtOpenFile`)하면 어떻게 될까? 라는 의문을 가지고 실험을 해보기 시작합니다. 실험 결과, 다음과 같은 사실을 발견하게 됩니다.
...
bool CreateNativeHardlink(LPCWSTR linkname, LPCWSTR targetname)
{
    ...
    // 대상파일을 오픈합니다.(DesiredAccess=MAXIMUM_ALLOWED)
...
`CreateHardLink`는 대상파일을 오픈할 때 요청한 권한이 `FILE_WRITE_ATTRIBUTES`인 반면, James Forshaw가 작성한 함수 `CreateNativeHardlink`는 `MAXIMUM_ALLOWED`를 사용한다 것이 가장 큰 차이점입니다. 
...
WRITE 권한이 없는 대상파일(예: c:\windows\notepad.exe)에 하드링크를 걸었다고 생각해봅시다. 하드링크를 통해서 대상파일(notepad.exe)을 WRITE할 수 있을까요? 만약 그렇게 된다면 바로 권한상승 취약점으로 이어질 수 있지만, 앞서 언급했듯이 하드링크는 디렉토리엔트리로서 그 자체로 퍼미션을 가지고 있지 않고 하드링크가 가리키는 파일이 퍼미션을 가지고 있기 때문에 하드링크를 통해서 WRITE 시도를 하게 되면, 그것이 가리키는 파일에 대한 접근검사가 이루어져서 WRITE 시도가 실패하게 되죠. Write권한이 없는 대상파일에 하드링크를 걸 수는 있으나, 하드링크를 통한 대상파일의 Write는 허용되지 않는 것입니다. 
