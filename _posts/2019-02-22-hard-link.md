...
여기서 눈여겨 볼 점은 대상 파일을 Open하는 데 `FILE_WRITE_ATTRIBUTES`을 인자로 사용한 것인데요. 만약 사용자가 대상 파일에 대해 `FILE_WRITE_ATTRIBUTES` 열기 권한이 없으면, `NtOpenFile` 호출은 실패하게 되고 결국 `CreateHardLink` 호출은 실패하게 됩니다.
...
James Forshaw는 이 지점에서 FILE_WRITE_ATTRIBUTES 없이 대상 파일을 Open(`NtOpenFile`)하면 어떻게 될까? 라는 의문을 가지고 실험을 해보기 시작합니다. 실험 결과, 다음과 같은 사실을 발견하게 됩니다.
...
`CreateHardLink`는 대상 파일을 오픈할 때 요청한 권한이 `FILE_WRITE_ATTRIBUTES`인 반면, James Forshaw가 작성한 함수 `CreateNativeHardlink`는 `MAXIMUM_ALLOWED`를 사용한다 것이 가장 큰 차이점입니다. 

`MAXIMUM_ALLOWED`는 어떻게든 파일을 열어달라는 의미로 보면 됩니다. 즉 *어떻게든 대상 파일을 열기만 하면 하드링크 생성이 가능하다*는 점을 코드를 통해 얘기해주고 있습니다.
