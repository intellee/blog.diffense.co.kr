---
title: About Windows Hardlink Abusing

summary: 하드링크란 무엇이며, 권한상승 취약점이 하드링크를 어떻게 활용되는지 살펴보고 대응 방안에 대해 얘기합니다.
...
mklink(CreateHardLink)를 이용하면 하드링크를 만들 수 없는 경우(예:kernel32.dll)가 있었는데, James Forshaw가 발견한 사실을 이용하면 그 경우에도 하드링크를 만들 수 있게 된 것이죠. 이를 증명하기 위해 다음과 같은 POC를 작성하여 공개[^2]하였습니다.
...
CreateHardLink API와의 차이점은 CreateHardLink는 타겟파일을 오픈할 때 요청한 권한이 FILE_WRITE_ATTRIBUTES인 반면, James Forshaw의 CreateNativeHardlink는 MAXIMUM_ALLOWED를 사용한다는 점입니다. MAXIMUM_ALLOWED는 어떻게든 파일을 열어달라는 의미로 보면 됩니다. 즉 *어떻게든 대상파일을 열기만 하면 하드링크 생성이 가능*함을 코드를 통해 말해주고 있습니다.
