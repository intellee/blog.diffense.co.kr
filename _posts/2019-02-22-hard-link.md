...
### 하드링크의 재미난 사실 
 ...
 윈도우즈 보안 연구로 굉장히 유명한 James Forshaw(구글 프로젝트 제로 소속)는 하드링크에 대한 흥미로운 사실[^2]을 발표하였습니다. 
...
여기서 주목할 부분은 `NtOpenFile`에 사용된 인자(`FILE_WRITE_ATTRIBUTES`)입니다. 만약 대상 파일에 대해 `FILE_WRITE_ATTRIBUTES` 로 파일을 오픈할 수 없다면, `NtOpenFile` 호출은 실패하게 되고 결국 `CreateHardLink` 호출은 실패하게 됩니다. 대상 파일을 속성쓰기 액세스로 Open하지 못하면 하드링크 생성을 못한다? 일견 타당해보이기도 하는데요. 

다음 예는 방금 언급한 `NtOpenFile` 호출이 실패하여 하드링크 생성에 실패하는 것을 보여주고 있습니다. 일반사용자는 `kernel32.dll` 파일을 `FILE_WRITE_ATTRIBUTES` 액세스로 Open할 수 없기 때문에 하드링크 생성에 실패한 것이죠.
...
James Forshaw는 이 지점에서 `FILE_WRITE_ATTRIBUTES` 인자를 빼고 대상 파일을 Open(`NtOpenFile`)해도 하드링크를 생성할 수 있을까?라는 의문을 가지고 실험한 결과 다음과 같은 사실을 발견하게 됩니다.
...
`CreateHardLink`는 대상 파일을 Open할 때 요청한 권한이 `FILE_WRITE_ATTRIBUTES`인 반면, James Forshaw가 작성한 함수 `CreateNativeHardlink`는 `MAXIMUM_ALLOWED`를 사용한다는 것이 가장 큰 차이점입니다. 
